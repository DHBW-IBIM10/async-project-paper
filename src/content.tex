...
Vert.x			& JavaScript, Java, Python, Groovy, Ruby, Coffeescript		
									& A JVM based platform for network applications which is
									  ...
									  Scala and Clojure is scheduled for future releases.\footcite[Cf.][]{vertx_2013}\\
\bottomrule 
\end{tabular*}
...
In Vert.x the smallest available deployment unit is a verticle, which runs inside an instance.
Multiple verticles can be run inside one instance as depicted in figure \ref{fig:vertx_constructs}
...
Vert.x includes a distributed event bus that spans the client and server side so
your applications components can communicate incredibly easily. The event bus
even penetrates into in-browser JavaScript allowing you to create effortless
so-called real-time web applications.


% Features
\begin{itemize}
  \item Concept of ``verticles'' (every instance is one verticle)
  \item Vert.x applications can be packaged to modules for reuse in other projects.
  Modules can be made public via a git repository at http://github.com/vert-x/vertx-mods 
  \item features a distributed event bus that can be shared among verticles
  		and the UI javascript
  \item worker threads for heavy tasks that would otherwise block the event 
  	loop as described in section \ref{sec:comparison}
  \item comes with modules that allow persistency, work queues, web server
  \item supports websockets and sockjs
  \item good scalability due to messaging passing, shared immutable data
  \item libraries can be embedded in own java or groovy application
  \item good coverage with documentation. Sample code for all supported languages and
most available features is available in a public git
repository\footcite[Cf.][]{Fox_2013}
  \item open source and licensed under the Apache Software License 2.0.
  	Commercial redistribution (in closed source projects) should not be an issue
\end{itemize}
