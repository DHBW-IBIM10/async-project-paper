...
\author{by Can Paul Bineytioglu,
		Joe Andr\'{e} Boden,
		Rocco Schulz,\\
		Max V\"{o}kler,
		Robert Wawrzyniak}
...
language=javascript,caption={Controlling streams in Node.js},label=lst:streaming]
...
language=Java,
caption={Verticle that starts two other verticles},
label={lst:starter_verticle},
morekeywords={}]{lst/starter_verticle.java}
...
caption={Command for starting the Vert.x application},
label={lst:vertx_start},
morekeywords={vertx}]{lst/vertx_start_command.sh}
...
\lstinputlisting[float,
language=JavaScript,
caption={Example of running \textit{ls -lh /usr} in Node.js, capturing stdout, stderr, and the exit code},
label=lst:node_child]{lst/node_child_process.js}
...
\lstinputlisting[float,
language=Java,
caption={Example of running \textit{ls -lh /usr} in Vert.x from within a worker verticle},
label=lst:vertx_child]{lst/vertx_child_process.java}
...
\lstinputlisting[float,
language=Java,
caption={Subclass of LogOutputStream as a handler for the stdout stream},
label=lst:java_stream]{lst/StdoutStream.java}
