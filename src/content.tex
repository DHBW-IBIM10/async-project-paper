...
\lstinputlisting[language=Bash,caption={Key generation using openssl},label=lst:openssl]{
...
The first Express functionality used is that static files can be served with node.js. Using the following statement, the directory ``/UI'' is made the root directory for a \lstinline$http://localhost:[port]/$

\lstinline[language=JavaScript]$app.use(express.static(__dirname + '/UI'));$
...
Listing \ref{vertx_child} shows a minimal worker verticle written in
...
The most natural way for inter-application communication is by using the integrated message bus.
...
well. \footcite{TODO: just a wild guess. find supporting documents}
