...
to the running server using the benchmarking tool Apache Bench\footnote{TODO:link}.
The benchmarking tool was configured to send 10,000 requests with a concurrency
level of 50 (50 requests in parallel).\\
Vert.x and Node.js were run single-threaded. Apache2 and Nginx were run in their
...
Apache2 had the worst respone times (y-axis), that were increasing rapidly with the number of requests sent (x-axis).
...
Vert.x. A partial plot is shown in figure \ref{fig:benchmark_async} for better comparison between Node.js and Vert.x.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{img/200kb_benchmark.png}
\caption[Plot of the web server benchmark results]{Plot of the web server benchmark results between Apache2, Nginx, Node.js, Vert.x (x-axis: requests, y-axis: time in ms)}
...
\caption[Plot of the web server benchmark results]{Plot of the web server benchmark results between Node.js and Vert.x (x-axis: requests, y-axis: time in ms)}
...
Vert.x and can be accomplished easily on a per-instance basis. Each instance can
