...
Node.js's asynchronous programming style is implemented through events that determine the low of execution. They are handeled by event handlers and event callbacks. Those 'functions' replace the usual result return allocation.
The asynchronous programming style in Node.js is accompanied by an event loop. This is a construct that mainly performs two functions in a continuous loop.  First, it detects events and second, it triggers event handler. In any run of the loop, it has to detect which events just happened. Then, when an event happens, the event loop must determine the event callback and invoke it. The whole event loop runs in one thread inside
one process, which causes no interruption in the event handler and will enable it to run to completion.
