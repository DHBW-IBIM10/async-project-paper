...
\caption{Processing Node.js\footnotemark}
...
<<<<<<< HEAD
Node.js' libraries are programmed completly asynchronous. It's architecture consists of modules to simplify the
creation of complex applications. Modules are encapsulated pieces of code like libraries in C or units in Pascal. Every module contains of a set
of sebject related functions. Defined functions and variables within a module are mostly private, meant they are 
only accessible from the inside of the module. Using a module requires to call the funktion require() and allocate it to a variable.
Every function in Node.js is asynchronous, which means that a callback function is run, after a specific I/O job is done. 
For example, if a file should be read on the file system, a specified callback function is executed when the read operation is completed.

The asynchronous programming style is implemented through events that determine the flow of execution. They are handeled by event handlers and event callbacks. Those 'functions' replace the usual result return allocation.
The asynchronous programming style in Node.js is accompanied by an event loop. This is a construct that mainly performs two functions in a continuous loop.  First, it detects events and second, it triggers event handler (callback). In any run of the loop, it has to detect which events just happened. Then, when an event happens, the event loop must determine the event callback and invoke it. The whole event loop runs in one process inside
one thread, which causes no interruption in the event handler and will enable it to run until the end.￼￼￼￼￼￼￼￼￼￼￼￼￼￼￼￼
This allows the programmer to relax the synchronization requirements and not have to worry about concurrent threads of execution changing the shared memory state.\footcite[Cf.][p.16]{teixeira_2012}

Out of the box, Node.js provides into the binary compiled core modules, to help you access files on the file system, create HTTP and TCP/UDP servers, and perform other useful functions.\footnote{See \url{http://nodejs.org/api/modules.html#modules_core_modules}} \\
...
The latest version of the Node.js environment can be installed from a GitHub repository.\footnote{See \url{http://nodejs.org/dist/latest/}} \\
>>>>>>> d98c4ae0d85902380e3681085519fa336a798364
Even though the core modules are sophisticated enough to write complex applications, Node.js is not complex itself.
But at the time of writing there were a massive of 21,308 easy to install packaed modules available to extend Node.js.\footcite[Cf.][TODO]{\url{https://npmjs.org}}
Since Node.js version 0.6.3 the package manager npm is deployed and installed automatically within the environment.
Npm is run through the command line and manages dependencies for applications.\\
As explained later on Node.js received standing ovations from the web developer community, which lead to spectacular participation in the project. In addition Node.js is supported by a San Francisco based company named Joyent, which offers cloud services such as Infrastructure as a Service and Platform as a Service to large companies, focussing on open source projects such as Ruby on Rails and Node.js.\footcite[Cf.][]{http://jaxenter.com/node-js-moves-to-joyent-32530.html}
Node.js's documentation and manual is extensive and covers all functionality of the non-extended edition.
...
Node.js's large performance benefits are caused through the use of Google's new JavaScript engine V8, which naturally ships with the web browser Chrome and is used for Node.js too. Due to the fact that V8 compiles JavaScript into
