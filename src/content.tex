...
popular webservers Apache2 and Nginx.
...
\caption[Plot of the web server benchmark results (asynchronous servers)]{Plot of the webserver benchmark results between Node.js and Vert.x (x-axis: requests, y-axis: time in ms)}
...
At the end the only real conclusion is that like all IT infrastructure asynchronous server technologies require careful evaluation of ones needs and wants and most off all concrete implementation goals in order to only use the most capable solution. Depending on the given problem, this might be Node.js, Vert.x or both (not necessarily in the same project, but that is definitely a possibility).
