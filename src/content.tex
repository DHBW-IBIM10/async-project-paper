...
% Features
\begin{itemize}
  \item Polyglot (multiple languages supported, can be mixed in one project)
  ...
  \item worker threads for heavy tasks that would otherwise block the event loop as described in section \ref{sec:comparison}
  ...
  \item easy to run multiple instances on one server, getting the most out of a 
  		multicore machine
...
repository\footcite[Cf.][]{Fox_2013}
  \item open source and licensed under the Apache Software License 2.0. Commercial redistribution (in closed source projects) should not be an issue
\end{itemize}
